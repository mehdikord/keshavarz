...
interface AdminRequestStatusChartProps {
  data: AdminDashboardKpis;
}

export function AdminRequestStatusChart({ data }: AdminRequestStatusChartProps) {
  const chartData = (
    Object.keys(REQUEST_STATUS_LABELS) as DashboardRequestStatus[]
  ).map((status) => ({
    key: status,
    label: REQUEST_STATUS_LABELS[status],
    value: data.requestsByStatus[status],
  }));
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            توزیع وضعیت‌ها در بازه انتخابی — بدون محاسبه سمت کلاینت
          </p>
        </div>
