...
import { purchaseProviderSubscription } from "@/server/modules/subscriptions/subscriptions.service";
...
export const POST = withApiHandler(async (request: NextRequest, context) => {
  ...
  const input = await parseJsonBody(request, PurchaseSubscriptionSchema);
  const idempotencyKey = parseWithSchema(
    IdempotencyHeaderSchema,
    request.headers.get(API_HEADERS.idempotencyKey),
  );
  const result = await purchaseProviderSubscription(
    auth.internalUserId,
    input,
    idempotencyKey,
  );
