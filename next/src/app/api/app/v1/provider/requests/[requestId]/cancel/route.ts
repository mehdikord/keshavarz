...
import { cancelProviderServiceRequest } from "@/server/modules/requests/request.service";
...
export const POST = withApiHandler(async (request: NextRequest, context) => {
  ...
  const input = await parseJsonBody(request, CancelRequestSchema);
  const result = await cancelProviderServiceRequest(
    auth.internalUserId,
    params.requestId,
    input,
  );
