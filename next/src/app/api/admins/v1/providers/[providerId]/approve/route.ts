...
import {
  AdminProviderApproveSchema,
  AdminProviderParamsSchema,
} from "@/server/modules/admin-providers/admin-providers.schemas";
import { approveProviderForAdmin } from "@/server/modules/admin-providers/admin-providers.service";
...
export const POST = withApiHandler(async (request: NextRequest, context) => {
  ...
  const input = await parseJsonBody(request, AdminProviderApproveSchema);
  const result = await approveProviderForAdmin({
